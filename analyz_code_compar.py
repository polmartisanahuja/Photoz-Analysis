...
for c in code_list:
	...
	for f in field_list:
		...
		zt, zp, m = np.loadtxt(folder_name + f + '/' + c + '_' + f + '_u.txt', usecols= (zt_col[j],zp_col[j],m_col[j]), unpack = True)
		...
		for e in estim_list:
			...
			if(e == 'sigma68'): np.savetxt('/Users/polstein/Desktop/' + c + '_' + f + '.txt' , np.array([x, val, err_val]).T)
